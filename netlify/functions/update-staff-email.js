...
exports.handler = async (event) => {
  ...
  const { accessToken, staffId, newEmail } = body;
  if (!accessToken || !staffId || !newEmail) {
  ...
  if (callerStaffErr || !callerStaff || !['admin', 'it'].includes(callerStaff.role)) {
    return { statusCode: 403, body: JSON.stringify({ error: 'Only Admin or IT can change staff emails' }) };
  }
  ...
  if (updateErr) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Could not update email: ' + updateErr.message }) };
  }

  return {
    statusCode: 200,
    body: JSON.stringify({ success: true, name: targetStaff.name, newEmail })
  };
};
